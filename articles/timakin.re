...
株式会社グノシー バックエンドエンジニアの@timakin@<fn>{timakin}です。
本章では、Goの画像処理パッケージを活用する方法を、現場での応用例を踏まえながら紹介します。

//footnote[timakin][@<href>{https://github.com/timakin}]

== imageパッケージの概要

Goの標準パッケージの一つである@<fn>{image}パッケージは、その名の通り2D画像を扱うためのパッケージです。
...
画像を新しく生成するには、@<code>{image.NewRGBA}を使います。
...
その中でもシンプルなインターフェースと必要十分な機能性を備えたパッケージとして、@<fn>{imaging}パッケージをご紹介します。
...
さて、@<code>{image}パッケージの概要を整理しましたが、次はそれらをどのように本章で活用するか、具体的な事例とその利用目的を説明します。
...
本章ではもう一歩踏み込んで、弊社で開発された@<fn>{オトクル}というクーポンが掲載されるアプリで活用した、画像生成の方法についてご紹介します。
...
また、固定で利用する素材hsサイズ的に問題がなければ@<fn>{statik}を使ってあらかじめバイナリに含めても良いと思います。
