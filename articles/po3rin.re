...
初めましてpo3rinです。業務ではGoをメインに使って開発しています。この章ではGoによる画像処理&画像解析の方法を紹介します。画像処理の入門はもちろん、標準パッケージのみを使った画像リサイズの実装や、画像や文字を合成する方法を紹介し、最後にはOpenCVを使った顔認識や輪郭抽出の実装を紹介します。この章を読み終わる頃にはGoでエレガントに画像を扱えるようになっていることでしょう。ソースコードは全て私のGitHubリポジトリ@<fn>{repo}に置いてあるので参考にどうぞ。ちなみにこの章のコードは実際の処理のみにフォーカスして紹介したい為、基本的にエラーハンドリングを省略するのでご了承ください。
...
また、@tottieさんの画像は金銭の発生する・しないに関わらず、許可なく二次利用することは出来ないので注意してください@<fn>{license}。
//footnote[license][正確には、金銭の発生する・しないに関わらず、二次利用する場合は原作者（Renée Frenchさん）とイラストの著作権保有者（@tottieさん）の許可を得た上で、著作権もとを記載する必要があります。]
...
@<list>{image_register}のように使う画像フォーマットをパッケージ内の@<code>{init}関数内で登録しています。よってinit関数を発火させる為のブランクインポートが必要なのです。
...
@<list>{grayscale}の通り、Goで画像加工をする際には、まず目的の画像(@<code>{dst}と呼ぶ)を作成し、元画像(@<code>{src}と呼ぶ)から1画素ずつ処理して@<code>{dst}にセットするという流れになります。今回においてはsrcから1画素値ずつ所得し、その色をグレースケール化して@<code>{dst}にセットしています。濃淡加工やセピア加工などのエフェクト処理においてもこのフローは鉄板なので覚えておきましょう。
...
ここからは更に実践的な内容に入りましょう。ブログを作成する際にOGP画像を作成するのってメンドくさいですよね。この節では画像やテキストの合成をGoで行い、OGP画像を自動で生成できるようにしましょう。なお次の章で@timakinさんが画像合成に関する実装のデザインパターンについて詳しく紹介していますので、この節では画像合成の仕組みにフォーカスして解説していきます。今回は@<img>{po3rin-ogp}のようなOGP画像生成を目指します。
...
では必要な素材を準備する関数を作りましょう。必要なのは@<code>{src}(Goのロゴ)と@<code>{cover}(背景画像)と@<code>{mask}(背景画像をマスキングする画像)と@<code>{dst}(合成を受ける画像)です。画像はそれぞれOGP画像の大きさに合うように前節で実装したリサイズ関数を使いましょう。前節の実装を飛ばした方は、全く同じリサイズ機能を提供する@<code>{github.com/po3rin/resize}パッケージを用意したのでそちらを使いましょう。
...
テキストの合成においては準標準パッケージも使っていきます。テキストをdstに合成する関数を作りましょう。今回は任意のフォントと色で画像にテキストを合成する方法を紹介します。
...
これでOGP画像を生成するコードができました！テキストとsrc画像を標準入力やフラッグで渡せるようにすれば、いろんなOGP画像がすぐに作れます。
...
輪郭抽出の結果を使って@<code>{gocv.DrawContours}関数(@<list>{drawContours})で輪郭の中を白色で塗りつぶします。@<code>{contours}は輪郭のデータです。第3引数@<code>{contourIdx}は描画したい輪郭のインデックス(第2引数で与えた輪郭のsliceから1つの輪郭だけを描画したいときに輪郭の指定に使います。全輪郭を描画する時は@<code>{ー1}を指定します。他の引数は文字通り色(@<code>{color})と線の太さ(@<code>{thickness})を表します。
...
本章ではGoで画像を扱う際の基本的な知識から、実装方法まで解説しました。Goで画像を自由自在に扱えるイメージを持っていただき、今後の開発に活かしていただけると幸いです。なお次の章で@timakinさんが画像合成実装のデザインパターンについて詳しく解説しているのでそちらも続けてどうぞ。
