...
初めましてpo3rinです。業務ではGoをメインに使って開発しています。この章ではGoによる画像処理の方法を紹介します。画像処理の入門はもちろん、標準パッケージのみを使った画像リサイズの実装や、画像や文字を合成する方法を紹介し、最後にはOpenCVを使った顔認識や輪郭抽出の実装を紹介します。この章を読み終わる頃にはGoでエレガントに画像を扱えるようになっていることでしょう。ソースコードは全て私のGitHubリポジトリ@<fn>{repo}に置いてあるので参考にどうぞ。ちなみにこの章のコードは実際の処理のみにフォーカスして紹介したい為、基本的にエラーハンドリングを省略するのでご了承ください。
