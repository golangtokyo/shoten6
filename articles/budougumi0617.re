...
今回の実装では、クリーンアーキテクチャ@<fn>{clean_arch}などのレイヤーによるパッケージングを使った設計パターンで扱うのが便利なリポジトリパターン@<fn>{repo_pattern}を用いて実装します。

//footnote[clean_arch][@<href>{https://amazon.jp/dp/B07FSBHS2V}]
...
=== リポジトリパターンを使った実装概要
リポジトリパターンはビジネスロジック、アプリケーションロジックからデータソースの永続化操作を分離する設計パターンです。
リポジトリパターンと、クリーンアーキテクチャなどのレイヤーによるパッケージングをベースにした設計パターンを組み合わせることで様々な恩恵を受けることができます。
まず、データソースの永続化操作を特定のレイヤー（今回は@<code>{repository}パッケージ）にカプセル化することができます。
その他のレイヤーはデータソースの永続化操作を考慮せず、ビジネスロジック・アプリケーションロジックのみを表現できます。
また、依存関係の挿入@<fn>{wiki_di}（@<tti>{Dependency Injection}、DI）でレイヤーを組み合わせることで、永続化操作の実装とビジネスロジック・アプリケーションロジックの依存関係を下げることができます。
...
通常のレイヤーによるパッケージングを用いた設計アプローチであれば、リポジトリから取得したデータモデルを使ってビジネスロジックを呼び出したり、アプリケーションロジックの実装を行ないます。
今回はサンプルなので、リポジトリからの取得したデータモデルをそのまま結果を返したり、ただデータモデルを生成するだけの薄い実装にします。
