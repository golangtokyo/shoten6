...
今回の実装では、クリーンアーキテクチャ@<fn>{clean_arch}などのレイヤー構造を持つアーキテクチャで扱うのが便利なリポジトリパターン@<fn>{repo_pattern}を用いて実装します。
...
リポジトリパターンと、クリーンアーキテクチャなどのレイヤー構造を持つアーキテクチャを組み合わせることで様々な恩恵を受けることができます。
...
通常のレイヤー構造を持つアーキテクチャを用いた設計アプローチであれば、リポジトリから取得したデータモデルを使ってビジネスロジックを呼び出したり、アプリケーションロジックの実装を行ないます。
