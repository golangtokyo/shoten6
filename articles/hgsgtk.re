...
ここで、「費用対効果」という考え方がでてきます。ユニットテストを効果的なものにするためには、ユニットテストによる効果をユニットテスト自体のコストより高い状態を目指す必要があります。つまり、かかるコストに対して得られる利益が大きい「費用対効果の高い」テストを意識して取り組むということです。

このユニットテストの効果とコストの関係性について、『xUnit Test Patterns: Refactoring Test Code』@<fn>{xutpLink}という書籍では、「テストの経済性（Economics of Test Automation）」という言葉で説明されています。@<img>{hgsgtk-test-economical-graph}をご覧ください。
...
//image[hgsgtk-test-economical-graph][費用対効果の高いテストの場合][scale=0.8]{
//}
...
//image[hgsgtk-test-uneconomical-graph][費用対効果の低いテストの場合][scale=0.8]{
//}

ソフトウェア開発のコスト全体を考慮すると、費用対効果の高いテストになるように意識することは非常に重要です。以降、費用対効果の高いユニットテストを目指していくために意識するべき点について説明します。
...
@<list>{ExectuteTestInStatusListNotReadable}の結果と比較して、エラーレポートから得られる情報量が増え、「@<code>{InStatusList}の検証を試み deleted という文字列を入力した際に false という結果が得られたが、期待値は true であった。」と直接的かつ十分な情報量を読み取ることができます。理想的には、保守するプログラマがテストの失敗を解明するためにソースコードを読む必要がないようにするべきです。
