...
なお、以降「テスト」と表記する場合、特に断りがない場合は「自動化されたユニットテスト」のことを指し示すと理解してください。

== Goのユニットテストの基本
まず、Goのユニットテスト作成の基本を抑えましょう。Goでは、 テストを行うためのコマンドとして、@<code>{go test} というサブコマンドが用意されています。@<code>{_test.go}というサフィックスの付いたファイルを対象にしてテストを実行します。
