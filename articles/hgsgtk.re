...
== ユニットテストとは
本章で扱う「ユニットテスト」という言葉について予め定義しておきます。ユニットテストとは、関数やメソッドレベルでの動作保証を行うテストです。ユニットテストを行う手段として「手動によるユニットテスト」・「自動化されたユニットテスト」の２種類があります。「手動によるユニットテスト」とは、事前に用意したユニットテスト仕様書をもとに人間の手によって行われるユニットテストです。本章では、「自動化されたユニットテスト」を扱います。自動化されたユニットテストでは、プログラムにより実行可能なテストコードを作成します。
