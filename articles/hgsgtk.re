...
この章では、ユニットテストについて扱います。ユニットテストを書くことは、業務でのアプリケーション開発にとって重要な要素のひとつです。ユニットテストの書き方についての記事は多数ありますが、"なぜその方法が良いのか"についての理解の場は少ないと感じています。そのため、本章では、"どのようなユニットテストが効果的か"と"それをどう実現するか"について説明します。この章を読むことで、普段何気なく書くユニットテストについて立ち止まって考える機会になることを期待しています。
...
テスト関数名は、 @<code>{Test} で始め、以降の接頭辞 @<code>{Name} は大文字で始める必要があります。また、引数定義には、@<code>{*testing.T}を設定します。また、パッケージ名はsample_testとしています。これについては@<hd>{外部テストパッケージ}にて詳細を説明します。
...
ここで、"費用対効果"の考え方が出てきます。ユニットテストがソフトウェア開発において効果的であるためには、ユニットテストによる効果がユニットテスト自体のコストより高い状態を保つ必要があります。つまり、かかるコストに対して得られる利益が大きい"費用対効果の高い"ユニットテストを意識するのです。
...
ユニットテスト自体のコストの発生について@<hd>{費用対効果の高いユニットテストとは}にて説明しました。発生するコストのひとつであるユニットテストの維持コストを最小限におさえるためには、適切なエラーレポートを意識する必要があります。
...
では、適切なエラーレポートとは何でしょうか。Goの公式FAQの"Why does Go not have assertions?"@<fn>{goFaqAssertions}では、"直接的かつ適切な内容"と説明されています。
具体的には、"施行した内容"・"どのような入力を行ったか"・"実際の結果"・"期待される結果"の４つの内容が含まれていることが望ましいとされています。

適切なエラーレポートを知るために、一度適切なエラーレポートができていない例を見てましょう。特定の文字列リストに引数が一致するか判定する@<code>{InStatusList()}に対するユニットテストを例として見ていきます。
...
エラーレポートに"どのような結果が得られたか"だけを含めました。このテストコードを実行した場合、@<list>{ExectuteTestInStatusListNotReadable}の結果が得られます。
...
さて、このエラーメッセージを見た時、何がわかったでしょうか。"なにやら期待していない false が返ってきたようだ"という情報は汲み取れます。しかし、"何の検証を試みて得られた結果なのか"・"どのような入力が与えられたのか"・"何が期待値なのか"についてエラーレポートからは分かりません。このエラーレポートを受け取ったプログラマは、失敗したテストコード・テスト対象の動作コードを慎重に読み解くことでしか何が起きているのか理解できません。ユニットテストが少ないうちはこのようなエラーレポートでも維持できるかもしれませんが、ケースが多くなってきて複数箇所でエラーが発生するようになると、このようなエラーレポートでは維持が苦しくなってきます。
では、より適切なエラーレポートをしているユニットテストを見てみましょう。
...
@<list>{ExectuteTestInStatusListNotReadable}の結果と比較して、エラーレポートから得られる情報量が増えました。エラーレポートから、@<code>{InStatusList()}の検証を試み、文字列"deleted" を入力した際に false が得られたが、期待値は true であった事がわかります。エラーレポートから、直接的かつ十分な情報量を読み取ることができました。理想的には、保守するプログラマがユニットテストの失敗を解明するためにソースコードを読む必要がないエラーレポートにするべきです。

== 適切なエラーハンドリング

Goの特徴的な言語仕様として、 アサーションが提供されていない点が挙げられます。これは、Goの公式FAQの"Why does Go not have assertions?"@<fn>{goFaqAssertions}にて理由が説明されていますが、"アサートが適切なエラーハンドリングとエラーレポートを考慮せずに済ますためのツールとして使われている"という懸念より提供されていません。このことからも、Goのユニットテストにおいて、適切なエラーハンドリングが重要なことがわかります。では、適切なエラーハンドリングとはなんでしょうか。適切なエラーハンドリングとは、致命的ではないエラーが発生した際にクラッシュさせずに処理を継続させることです。
...
ひとつ、@<code>{*testing.T.Fatalf}と@<code>{*testing.T.Errorf}を使い分ける例として、OKを返すHTTPハンドラー@<code>{OkHandler()}に対するユニットテストを見てみましょう。
...
@<list>{OkHandler}の@<code>{OkHandler()}に対して、@<code>{*testing.T.Fatalf}と@<code>{*testing.T.Errorf}両方を使用してテストコードを書きます。
...
@<list>{TestOkHandler}のテスト関数@<code>{TestOkHandler}では、テスト対象のHTTPハンドラである@<code>{OkHandler()}のレスポンスボディを@<code>{ioutil.ReadAll()}で読み取った際にエラーが発生するケースがあります。ここでエラーが発生した場合は後続のレスポンスボディの値検証は不可なため、@<code>{*testing.T.Fatalf}を使ってテストをクラッシュさせています。

ユニットテストにおいて致命的なエラーの一例として、テスト対象を実行・検証する前準備の処理の失敗が挙げられます。このような、処理の継続が不可能なものに対してはユニットテストをクラッシュさせることが効果的です。
...
適切なエラーハンドリングを行うことによって、一回のユニットテスト実行で多くのエラー結果を得ることができます。この利点を実感するために、適切なエラーハンドリングができていない例を見てましょう。数値に対してFizzBuzz判定をする@<code>{fizzbuzz.GetMsg()}に対するユニットテストを見ていきます。
...
すべてのエラーを、@<code>{*testing.T.Fatalf}でハンドリングしています。ユニットテストが通っている間はこれでも問題ないように見えるかもしれません。では、@<list>{FizzBuzzGetMsg}の@<code>{fizzbuzz.GetMsg()}に２箇所バグが侵入してしまったケースを考えてみましょう。
...
@<list>{BugFizzBuzzGetMsg}の変更によって２箇所のバグが侵入しました。ここで、@<list>{TestFizzBuzzGetMsg}の@<code>{TestGetMsg()}を実行してみましょう。
...
最初の１箇所のみがエラー結果として得られました。もう１つ侵入したバグに対するエラー結果を得ることはできませんでした。このテストコードでは、処理の継続にとって致命的なエラーではないものも、すべて@<code>{*testing.T.Fatalf}でハンドリングしています。結果、１回の実行によって得られる情報が少ないため、"何度も修正しては現れる別のエラーに対して修正を繰り返さなければならない"状況を作ってしまう可能性があります。もし、@<code>{*testing.T.Errorf}でハンドリングしていた場合は@<list>{ExecuteTestFizzBuzzGetMsgBetter}のように１回で多くの情報を得ることができます。
...
このように、適切なエラーハンドリングが行われておらず全てのエラーでクラッシュしていた場合、プログラマはエラーを直さない限り、クラッシュした箇所以降の検証結果がわかりません。一回の実行結果から得られる情報量がすくないため、保守するプログラマは"都度エラーを直しては次のエラーを調査する"といった方法を強いられることになります。ユニットテストの維持コストを鑑みても、適切なエラーハンドリングは意識する必要があります。

== テーブル駆動テスト・サブテスト

=== テーブル駆動テスト

Goでは非常に広く使われているユニットテストの技法として、テーブル駆動テスト@<fn>{wikiTableDrivenDevelopment}があります。テーブル駆動テストでは、入力・期待値を含めたテストケースの一覧をテーブルとして作成します。早速、@<list>{FizzBuzzGetMsg}で例としてあげた@<code>{fizzbuzz.GetMsg}に対するユニットテストを、テーブル駆動テストで書いてみましょう。
...
テーブル駆動テストでは、必要に応じた新たな表の項目を追加するのが簡単であり、判定ロジックの複製が不要です。そのため、テストコードに対する修正・追加コストの低いユニットテストを実現する方法になります。あわせて、テストコードを読む際にも、対象関数のテストパターンが表にかかれているので可読性の高いユニットテストとなります。さらに、判定ロジックの複製が不要な分、適切なエラーレポート・エラーハンドリングに集中することができます。
...
サブテストを活用することで、それぞれのテストケースにテストケース名を与えれます。これによって、テストコードを読むプログラマに対して意図を伝えやすくなります。また、特定のサブテストのみを実行できることで、デバッグ時に必要最小限のユニットテスト実行で済むためデバッグコストの削減に繋がります。
...
費用対効果の高いユニットテストを目指す上で、テストコードの可読性やテストコードの追加のしやすさは重要な要素です。テストコードの可読性はドキュメンテーションとしての価値を高め、追加のしやすさは新規テストコードの作成コストの削減に繋がります。これら２つを目指すためのひとつの方法として、テストヘルパーを作成・利用する方法があります。実際に使用する例として明日の時間を返す@<code>{GetTomorrow()}という関数に対するユニットテストを見ていきましょう。
...
テストコードを置くパッケージについて２つの選択肢があります。テスト対象と同一のパッケージにするか、外部テストパッケージにする２つの選択肢です。外部テストパッケージは、"xxx_test"の形式のパッケージ名になり、たとえば sample パッケージであれば sample_test というパッケージ名です。
...
費用対効果の高いユニットテストを目指すために、テストパッケージはどちらの方法を選択するべきなのでしょうか。これを考えるための材料として、ブラックボックステスト・ホワイトボックステストの２つの概念をおさえましょう。

ユニットテストは、"テスト対象の内部の動きをどれだけ知っているか"によって、ブラックボックステスト・ホワイトボックスの２種類に分類できます。ブラックボックステストは、パッケージがAPIとして公開している機能以外はそのパッケージについて関して何も知らないことを前提とします。対象的にホワイトボックステストはパッケージの内部機能にアクセスできます。外部テストパッケージの場合、非公開機能へのアクセスのしにくさによりブラックボックステストになりやすいでしょう。一方、同一パッケージの場合、非公開機能へのアクセスが可能になるため、ホワイトボックステストを行いやすくなります。

ブラックボックステストは、"壊れやすいテスト（Fragile Test）"@<fn>{fragileTest}になりにくい利点があります。壊れやすいテストとは、動作コードの変更に敏感に反応して失敗してしまうユニットテストのことです。"敏感に"とは、外部への振る舞いを変えない内部の処理詳細の変更に対してもユニットテストが失敗してしまう状況を表しています。"変更検出（change detecter）テスト"あるいは"現状維持（status quo）テスト"と呼ばれることもあります。壊れやすいテストは、必要以上の変更頻度を発生させ、ユニットテストの維持コストの増大に繋がります。ブラックボックステストの場合は、公開機能のみ知っているという前提があるため、テスト対象パッケージと疎結合になりやすく、壊れやすいテストの発生確率が低いという利点があります。

//footnote[fragileTest][@<href>{http://xunitpatterns.com/Fragile%20Test.html}]
...
テストパッケージをどちらにするのかはこれらのどちらの利点を重視するかによって変わるでしょう。筆者は、壊れやすいテスト発生による維持コストの増加の懸念とパッケージのクライアントの視点の２つの利点を重視するケースが多いです。そのため、ブラックボックステストになりやすい外部テストパッケージとしてテストコードを置くようにしています。そのうえで、非公開機能に対して公開機能を介さずに直接ユニットテストを書きたい場合には、非公開機能を別パッケージの公開機能として切り出せないか検討する方針としています。
