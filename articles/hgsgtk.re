...
テスト関数名は、 @<code>{Test} で始め、以降の接頭辞 @<code>{Name} は大文字で始める必要があります。また、引数定義には、@<code>{*testing.T}を設定します。また、パッケージ名は@<code>{sample_test}としています。これについては@<hd>{外部テストパッケージ}にて詳細を説明します。
...
テストコードを置くパッケージについて２つの選択肢があります。テスト対象と同一のパッケージにするか、外部テストパッケージにする２つの選択肢です。外部テストパッケージは、@<code>{xxx_test}の形式のパッケージ名になり、たとえば sample パッケージであれば sample_test というパッケージ名です。
...
テストパッケージをどちらにするのかはこれらのどちらの利点を重視するかによって変わるでしょう。筆者は、壊れやすいテスト発生による維持コストの増加の懸念とパッケージのクライアントの視点の２つの利点を重視するケースが多いです。そのため、ブラックボックステストのなりやすい外部テストパッケージにテストコードを置くようにしています。そのうえで、非公開機能に対して公開機能を介さずに直接ユニットテストを書きたい場合には、非公開機能を別パッケージの公開機能として切り出せないか検討する方針としています。
