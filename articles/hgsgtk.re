...
ここで、"費用対効果"という考え方がでてきます。ユニットテストがソフトウェア開発において効果的であるためには、ユニットテストによる効果がユニットテスト自体のコストより高い状態を保つ必要があります。つまり、かかるコストに対して得られる利益が大きい"費用対効果の高い"ユニットテストを意識するという考え方です。

このユニットテストの効果とコストの関係性について、『xUnit Test Patterns: Refactoring Test Code』@<fn>{xutpLink}という書籍では、"Economics of Test Automation"という言葉で説明されています。
...
ユニットテスト自体のコストの発生について"費用対効果の高いユニットテストという考え方"にて説明しました。その中でも、ユニットテストの維持コストを最小限におさえるために意識するべき点のひとつとして、適切なエラーレポートという観点があります。これは、ユニットテストが失敗した際に"なぜ失敗したのか"について、それを見るプログラマに対してわかりやすいレポートができているかという観点です。わかりやすいレポートにすることによって、ユニットテストの失敗に対する調査・解決にかかるコストが下げり、ユニットテストを維持するコストの削減に繋がります。
...
さて、このエラーメッセージを見た時、何がわかったでしょうか。"なにやら期待していない false が返ってきたようだ"という情報は汲み取れます。しかし、"何の検証を試みて得られた結果なのか"・"どのような入力（引数）が与えられたのか"。"何が期待値なのか"についてエラーレポートからは分かりません。このエラーレポートを受け取ったプログラマは、失敗したテストコード・テスト対象の動作コードを慎重に読み解くことでしか何が起きているのかが理解できません。テストケースが少ないうちはこのようなエラーレポートでもメンテナンスできるかもしれませんが、ケースが多くなってきて複数箇所でエラーが発生するようになるとこのようなエラーレポートではメンテナンスが苦しくなってきます。
...
@<list>{ExectuteTestInStatusListNotReadable}の結果と比較して、エラーレポートから得られる情報量が増え、"@<code>{InStatusList()}の検証を試み deleted という文字列を入力した際に false という結果が得られたが、期待値は true であった。"と直接的かつ十分な情報量を読み取ることができます。理想的には、保守するプログラマがユニットテストの失敗を解明するためにソースコードを読む必要がないエラーレポートにするべきです。
